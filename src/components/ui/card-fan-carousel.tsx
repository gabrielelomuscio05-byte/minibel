  ...
  useEffect(() => {
    ...
    const updateHoverLayout = (hoveredSlot: number | null) => {
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        const distance = hoveredSlot !== null ? Math.abs(slot - hoveredSlot) : 0;
        const delay = hoveredSlot !== null 
          ? distance * 0.02 
          : Math.abs(slot - centerSlot) * 0.02;
            ...
            if (card.onClick) {
              return (
                <button 
                  ...
                  className="fan-card block cursor-pointer border-0 bg-transparent p-0 text-left focus:outline-none focus:ring-2 focus:ring-black rounded-3xl"
                >
                  {image}
                </button>
              );
